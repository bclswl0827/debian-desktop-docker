...
if [ -n "$RESOLUTION" ]; then
    sed -i "s/1366x768/$RESOLUTION/" /usr/local/bin/xvfb.sh
fi
...
if [ "$USER" != "root" ]; then
    ...
    if [ -z "$PASSWORD" ]; then
        echo "  set default password to \"passw0rd\""
        PASSWORD=passw0rd
    fi
    HOME=/home/$USER
    echo "$USER:$PASSWORD" | chpasswd
    cp -r /root/{.config,.local,.gtkrc-2.0,.asoundrc} ${HOME}
    chown -R $USER:$USER ${HOME}
    chattr +i ${HOME}/.local/share/Trash/
fi
